...
const Signup = ({ contracts, setAuthentication, history }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (contracts.type == "User") {
      ...
      const perso = await contracts.Factory.methods
        .createPerson(name, aadhar)
        .send({ from: accounts[0] });
      const addressContract = await contracts.Factory.methods
        .people(accounts[0])
        .call();
      const personContract = new contracts.web3.eth.Contract(
        Person.abi,
        addressContract
      );
  ...
  return !loading ? (
    <div>
      ...
      <div className="jumbotron color-grey-light mt-70 mb-0">
        <div
          className="d-flex align-items-center h-50"
          style={{ backgroundImage: `url(${image1})` }}
        >
          <div className="container text-center py-5">
            {/*Grid row*/}
            <h2 className="font-weight-bold text-white mb-0 text-center text-md-left">
              Form
            </h2>
            {/*Grid row*/}
          </div>
            ...
            <div className="cell-sm-10 cell-md-7 cell-lg-6">
              ...
              >
                <div className="range range-sm-bottom range-20">
                  ...
                  <div className="cell-sm-6">
                    <button
                      className="button button-block button-primary submit"
                      type="submit"
                    >
                      Register
                    </button>
...
export default connect(mapStateToProps, { setAuthentication })(
  withRouter(Signup)
);
